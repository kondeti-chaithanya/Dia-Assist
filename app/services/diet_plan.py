def get_age_group(age: int) -> str: #Determine age group
    if age <= 9:
        return "child"
    elif age <= 17:
        return "adolescent"
...
def calculate_daily_calories(age, gender, bmi): #Calculate daily calorie needs
...
def calculate_macros(calories, age_group, diabetic):#Calculate macronutrient distribution
    ...
    fat_pct = 1 - (carb_pct + protein_pct) #fat calculation
...
def get_diet_plan(user: dict): #This is the entry point used by FastAPI.
    age = user["age"]
    gender = user["gender"]
    bmi = user["bmi"]
    ...
    # ---------------- MEAL PLAN ----------------
    meal_plan = { #Frontend can toggle veg / non-veg/ Easy to extend
        "breakfast": {
            "veg": [
