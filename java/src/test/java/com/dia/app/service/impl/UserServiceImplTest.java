...
class UserServiceImplTest {
    ...
    void register_savesUser_andReturnsMessage() {
        ...
        assertEquals("Register Sucessful", result);
    ...
    void login_success_returnsTokenAndUser() {
        ...
        when(jwtService.generateToken(user.getEmail(), user.getId())).thenReturn("tok123");

        Map<String, Object> resp = userService.login("me@example.com", raw);

        assertNotNull(resp);
        assertEquals("tok123", resp.get("token"));
        assertTrue(resp.get("user") != null);
    }
