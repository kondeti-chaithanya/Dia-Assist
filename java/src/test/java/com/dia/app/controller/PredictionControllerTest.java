...
class PredictionControllerTest {
    ...
    void predict_requiresAuthentication() {
        ...
        var resp = predictionController.predict(req, null, "Bearer t");
    ...
    void predict_withAuthentication_callsService() {
        ...
        when(predictionService.predictAndGenerateDiet(any(), any(), any())).thenReturn(dto);

        // create a simple Authentication mock
        var auth = new org.mockito.Mockito().mock(org.springframework.security.core.Authentication.class);
        when(auth.getName()).thenReturn("me@example.com");

        var resp = predictionController.predict(req, auth, "Bearer token");
