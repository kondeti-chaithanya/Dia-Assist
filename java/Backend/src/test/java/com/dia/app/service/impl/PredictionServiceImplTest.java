...
class PredictionServiceImplTest {
    ...
    void predictAndGenerateDiet_savesPrediction_andReturnsDto() {
        ...
        PredictionResponseDTO dto = predictionService.predictAndGenerateDiet(req, user);

        assertNotNull(dto);
        assertEquals(1, dto.getPrediction());
        assertEquals("ok", dto.getMessage());
        assertEquals("reason", dto.getWhyThisResult());
    ...
    void callPythonPrediction_delegatesToClient() {
        // This test was for an older API that used PythonClientService directly.
        // The current implementation delegates to `PythonService` and has no
        // `callPythonPrediction` method, so this test is no longer applicable.
    }
}
